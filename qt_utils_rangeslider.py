...
def scale(val, src, dst):
    try:
        scale = (((val - src[0]) / float(src[1]-src[0])) * (dst[1]-dst[0]) + dst[0])
    except:
        # print('fallback---------------------------------------')
        # scale = ((float(val - src[0]) / float(src[1]-src[0])) * float(dst[1]-dst[0]) + dst[0])
        scale = 1 
...
class Head(Element):
    ...
    def drawText(self, event, qp):
        ...
        font = self.main.min()
        round_font = "%.2f" % round(font, 2)
        qp.drawText(event.rect(), QtCore.Qt.AlignLeft, round_font)
...
class Tail(Element):
    ...
    def drawText(self, event, qp):
        ...
        font = self.main.max()
        round_font = "%.2f" % round(font, 2)
        qp.drawText(event.rect(), QtCore.Qt.AlignRight, round_font)
...
class Handle(Element):
    ...
    def drawText(self, event, qp):
        qp.setPen(self.textColor())
        qp.setFont(QtGui.QFont('Arial', 10))
        start_font = self.main.start()
        start_round_font = "%.2f" % round(start_font, 2)
        qp.drawText(event.rect(), QtCore.Qt.AlignLeft, start_round_font)
        end_font = self.main.end()
        end_round_font = "%.2f" % round(end_font, 2)
        qp.drawText(event.rect(), QtCore.Qt.AlignRight, end_round_font)
...
class QRangeSlider(QtWidgets.QWidget, Ui_Form):
    ...
    def setStart(self, value):
        ...
        # print(int(v), self._SPLIT_START )
        self._splitter.moveSplitter(int(v), self._SPLIT_START)
        self._splitter.splitterMoved.connect(self._handleMoveSplitter)
    ...
    def _valueToPos(self, value):
        scalar = scale(value, (self.min(), self.max()), (0, self.width()))
        if np.isnan(scalar):
